	...
	public function saveturnover(){
        ...
        elseif($this->input->post('saveyearlyturnover')!==NULL){
            ...
            if($query->num_rows()>0){
                ...
                unset($data['id'],$data['year'],$data['month'],$data['turnover'],$data['due_date'],
                ...
                foreach($months as $key=>$month){
                    ...
                    if($getprev->num_rows()==0){
                        ...
                        $getpayment=$this->db->get_where('acc_payment',
                    ...
                    if($query->num_rows()>0){
                        ...
                        foreach($firm_ids as $firm_id){
                            foreach($turnovers[$user_id][$firm_id] as $key=>$turnover){
                                ...
                                $data['added_on']=$data['updated_on']=date('Y-m-d H:i:s');
                                $getprev=$this->db->get_where('accountancy',['date'=>$data['date'],'user_id'=>$user_id,
                                                                             'firm_id'=>$firm_id]);
                                //print_pre($data);
                                if($getprev->num_rows()==0){
                                    $result=$this->service->saveturnover($data);
        ...
        if(!empty($year)){
            ...
            if(!empty($turnovers)){
                foreach($turnovers as $single){
                    ...
                    $row=array('index1'=>$single['user_id'],'index2'=>$single['firm_id'],'monthindex'=>$month,
