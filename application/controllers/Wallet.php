    ...
    public function viewearnings($emp_id = NULL)
    {
        ...
        if (!empty($data['earnings'])) {
            foreach ($data['earnings'] as $earning) {
                $total_earnings += !empty($earning['amount']) ? $earning['amount'] : 0;
                $total_orders++;
            }
        }

        // Get employee ID from md5 hash to fetch balance
        $emp_query = $this->db->select('id')->where('md5(id)', $emp_id)->get('employees');
        $actual_emp_id = NULL;
        if ($emp_query->num_rows() > 0) {
            $actual_emp_id = $emp_query->row()->id;
        }

        // Get employee balance (total earnings - total payments)
        $total_payments = 0;
        $balance = 0;
        if ($actual_emp_id) {
            $balance_info = $this->employee->getemployeebalance($actual_emp_id);
            $total_payments = $balance_info['payments'];
