    ...
    public function getwallet_post(){
        ...
        if(!empty($token)){
            ...
            if(!empty($user) && is_array($user) && $user['role']=='customer'){
                ...
                $this->response([
                        'status' => true,
                        'balance' => $balance], RestController::HTTP_OK);
            }		
            else{
                $this->response([
                    'status' => false,
            ...
            if(!empty($user) && is_array($user) && $user['role']=='customer'){
                ...
                if($array['code']=='PAYMENT_SUCCESS'){
                    ...
                    if(!empty($wallet)){
                        ...
                        $this->response([
                            'status' => false,
                            'message' => "Transaction not Found! Try Again!"], RestController::HTTP_OK);
                    }
                }
                else{
                    $this->response([
                        'status' => false,
                        'message' => "Payment Failed! Try Again!"], RestController::HTTP_OK);
                }
            }		
            else{
                $this->response([
            ...
            if(!empty($user) && is_array($user) && $user['role']=='customer'){
                ...
                if($query->num_rows()>0){
                    ...
                    if(!empty($accountancy)){
                        ...
                        $this->response([
                            'status' => true,
                            'total_dues' => $total_dues], RestController::HTTP_OK);
                    }
                    else{
                        $this->response([
                            'status' => false,
                            'message' => "No Dues!"], RestController::HTTP_OK);
                    }
                }		
                else{
                ...
                if($balance>=$amount){
                    ...
                    if($query->num_rows()>0){
                        ...
                        if(!empty($accountancy)){
                            ...
                            $total_dues=$total_fees+$total_penalty-$total_paid;
                            $data=array('date'=>$date,'user_id'=>$user_id,'firm_id'=>$firm_id,'acc_date'=>$currentmonth,
                                        'amount'=>$amount);
